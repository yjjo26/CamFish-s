...
('이마트24 인천공항점', 'AMENITY', '인천 중구 공항로', ST_SetSRID(ST_MakePoint(126.4500, 37.4500), 4326), '공항 근처 대형 편의점'),
('만정수산', 'AMENITY', '인천 중구 영종순환로', ST_SetSRID(ST_MakePoint(126.5500, 37.5000), 4326), '왕새우구이 맛집'),
('영종도 마시안 제빵소', 'AMENITY', '인천 중구 마시란로', ST_SetSRID(ST_MakePoint(126.4000, 37.4300), 4326), '오션뷰 베이커리 카페'),
...
('가평휴게소 (춘천방향)', 'AMENITY', '경기 가평군 설악면 미사리로 540', ST_SetSRID(ST_MakePoint(127.4810, 37.7600), 4326), '가평 잣호두과자, 맛남샌드'),
('가평휴게소 (서울방향)', 'AMENITY', '경기 가평군 설악면 미사리로 544', ST_SetSRID(ST_MakePoint(127.4815, 37.7605), 4326), '가평 잣소고기국밥'),

-- 속초/양양/강릉권 (동해안)
('속초 이마트', 'AMENITY', '강원 속초시 설악금강대교로', ST_SetSRID(ST_MakePoint(128.5900, 38.1900), 4326), '속초 여행 장보기 필수 코스'),
('속초 중앙시장', 'AMENITY', '강원 속초시 중앙로', ST_SetSRID(ST_MakePoint(128.5900, 38.2000), 4326), '닭강정, 오징어순대'),
('봉포머구리집', 'AMENITY', '강원 속초시 영랑해안길', ST_SetSRID(ST_MakePoint(128.5950, 38.2200), 4326), '유명 물회 맛집'),
('양양 서퍼스 편의점', 'AMENITY', '강원 양양군 현남면', ST_SetSRID(ST_MakePoint(128.7600, 37.9700), 4326), '죽도해변 서퍼들의 쉼터'),
('강릉 테라로사 커피공장', 'AMENITY', '강원 강릉시 구정면', ST_SetSRID(ST_MakePoint(128.8900, 37.7000), 4326), '강릉 커피의 본산'),
('주문진 수산시장', 'AMENITY', '강원 강릉시 주문진읍', ST_SetSRID(ST_MakePoint(128.8300, 37.8900), 4326), '싱싱한 회와 건어물'),
('내린천 휴게소 (양양방향)', 'AMENITY', '강원 인제군 상남면', ST_SetSRID(ST_MakePoint(128.2500, 37.9500), 4326), '국내 최초 상공형 휴게소'),

-- 부산/기장권
('기장 롯데아울렛', 'AMENITY', '부산 기장군 기장읍', ST_SetSRID(ST_MakePoint(129.2200, 35.1900), 4326), '쇼핑과 먹거리, 주차장 완비'),
('칠암붕장어마을', 'AMENITY', '부산 기장군 일광면', ST_SetSRID(ST_MakePoint(129.2600, 35.2800), 4326), '붕장어(아나고) 회/구이'),
('해운대 암소갈비집', 'AMENITY', '부산 해운대구', ST_SetSRID(ST_MakePoint(129.1600, 35.1600), 4326), '부산 대표 갈비 맛집'),

-- 여수/남해권
('여수 돌산갓김치 판매장', 'AMENITY', '전남 여수시 돌산읍', ST_SetSRID(ST_MakePoint(127.8000, 34.6000), 4326), '여수 특산품 구매'),
('여수 낭만포차 거리', 'AMENITY', '전남 여수시 종화동', ST_SetSRID(ST_MakePoint(127.7400, 34.7300), 4326), '여수 밤바다와 삼합'),
('황소식당', 'AMENITY', '전남 여수시 봉산동', ST_SetSRID(ST_MakePoint(127.7200, 34.7300), 4326), '게장 백반 맛집'),

-- 낚시점 (주요 거점)
('속초 낚시프라자', 'AMENITY', '강원 속초시 동명동', ST_SetSRID(ST_MakePoint(128.6000, 38.2100), 4326), '동명항 인근 대형 낚시점'),
('군산 낚시창고', 'AMENITY', '전북 군산시 비응도동', ST_SetSRID(ST_MakePoint(126.5100, 35.9300), 4326), '비응항 선상 낚시 채비'),
('목포 목포낚시', 'AMENITY', '전남 목포시 북항로', ST_SetSRID(ST_MakePoint(126.3800, 34.8100), 4326), '갈치, 쭈꾸미 채비 전문'),
('부산 본낚시', 'AMENITY', '부산 사하구 다대동', ST_SetSRID(ST_MakePoint(128.9700, 35.0500), 4326), '다대포 갯바위 전문'),
('제주 낚시센터', 'AMENITY', '제주 제주시 탑동로', ST_SetSRID(ST_MakePoint(126.5200, 33.5200), 4326), '제주 전역 출조 문의'),

-- 고속도로 휴게소 (경유지 추천용)
('시흥하늘휴게소', 'AMENITY', '경기 시흥시 조남동', ST_SetSRID(ST_MakePoint(126.8600, 37.3800), 4326), '서울외곽순환 브릿지 스퀘어'),
('행담도휴게소', 'AMENITY', '충남 당진시 신평면', ST_SetSRID(ST_MakePoint(126.6800, 36.9500), 4326), '서해안고속도로 아울렛 겸용'),
('횡성휴게소 (강릉방향)', 'AMENITY', '강원 횡성군 안흥면', ST_SetSRID(ST_MakePoint(128.1400, 37.4500), 4326), '횡성한우 국밥'),
('금강휴게소', 'AMENITY', '충북 옥천군 동이면', ST_SetSRID(ST_MakePoint(127.6700, 36.2800), 4326), '금강이 보이는 휴게소 (도리뱅뱅)'),
('덕평자연휴게소', 'AMENITY', '경기 이천시 마장면', ST_SetSRID(ST_MakePoint(127.3500, 37.2100), 4326), '별빛정원 우주, 강아지 파크')
